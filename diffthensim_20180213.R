...
df <- foreach(i=1:nsim, .combine = "rbind", .export=ls(envir=globalenv()))  %:%  
    ...
    a.noimp.mar <- coefficients(noimp.mar_M)[2] 
    b.noimp.mar <- coefficients(noimp.mar_Y)[3]
...
library(plyr)    
singleid.data <- data.complete_wide[,c("id","M.mcar.1","M.mar.1")]

combinedData <- join(data.complete, singleid.data, by='id', type='left', match='all')
combinedData$M_M1.mcar <- combinedData$M.mcar - combinedData$M.mcar.1
combinedData$M_M1.mar <- combinedData$M.mar - combinedData$M.mar.1

mcar.data <- combinedData[,c("group","X1","X2")] 
mcar.X <- cbind(mcar.data[,0, drop=FALSE], 1, mcar.data[,1:3] ) # create design matrix with 1s in column 1
names(mcar.X)[1] <- "int"

time <- model.matrix( ~ as.factor(time), data=combinedData)[,2:6]

pred <- cbind(mcar.X$int, mcar.X$group, time, mcar.X$X1, mcar.X$X2, mcar.X$group*time)

xcol <- 1:14
zcol <- 1
Y.mcar <- as.vector(combinedData$Y.mcar)
M_M1.mcar <- as.vector(combinedData$M_M1.mcar)
YM_M1.mcar <- cbind(Y.mcar, M_M1.mcar)
subj <- as.vector(combinedData$id)
...
model.em_mcar <- mlmmm.em(y=YM_M1.mcar, subj, pred, xcol, zcol, maxits = 400, eps = 0.0001)

ad = r = c = 2 
Binv_mcar <- ad * model.em_mcar$sigma
Dinv_mcar <- c * model.em_mcar$psi

prior_mcar <- list(a=ad, Binv=Binv_mcar, c=c, Dinv=Dinv_mcar) # setting priors

# Conducting diagnostics to determine the burn-in

# mcar.YM_pan <- pan(y=YM_M1.mcar, subj, pred, xcol, zcol, prior_mcar, seed=194,iter=5000)
# 
# plot(1:5000,log(mcar.YM_pan$sigma[1,1,]),type="l", main="Variance for Y")
# acf(log((mcar.YM_pan$sigma[1,1,])))
# 
# plot(1:5000,log(mcar.YM_pan$sigma[2,2,]),type="l", main="Variance for M_M1")
...
mcar.YM_pan <- pan(y=YM_M1.mcar,subj, pred, xcol, zcol, prior_mcar,seed=i+194,iter=5000)
mcar.YM1 <- mcar.YM_pan$y
mcar.YM_pan <- pan(y=YM_M1.mcar,subj, pred, xcol, zcol, prior_mcar,seed=i+257,iter=100,start=mcar.YM_pan$last)
mcar.YM2 <- mcar.YM_pan$y
mcar.YM_pan <- pan(y=YM_M1.mcar,subj,pred, xcol, zcol, prior_mcar,seed=i+483,iter=100,start=mcar.YM_pan$last)
mcar.YM3 <- mcar.YM_pan$y
mcar.YM_pan <- pan(y=YM_M1.mcar,subj,pred, xcol, zcol, prior_mcar,seed=i+106,iter=100,start=mcar.YM_pan$last)
mcar.YM4 <- mcar.YM_pan$y
mcar.YM_pan <- pan(y=YM_M1.mcar,subj,pred, xcol, zcol, prior_mcar,seed=i+835,iter=100,start=mcar.YM_pan$last)
mcar.YM5 <- mcar.YM_pan$y
mcar.YM_pan <- pan(y=YM_M1.mcar,subj,pred, xcol, zcol, prior_mcar,seed=i+876,iter=100,start=mcar.YM_pan$last)
mcar.YM6 <- mcar.YM_pan$y
mcar.YM_pan <- pan(y=YM_M1.mcar,subj,pred, xcol, zcol, prior_mcar,seed=i+661,iter=100,start=mcar.YM_pan$last)
mcar.YM7 <- mcar.YM_pan$y
mcar.YM_pan <- pan(y=YM_M1.mcar,subj,pred, xcol, zcol, prior_mcar,seed=i+163,iter=100,start=mcar.YM_pan$last)
mcar.YM8 <- mcar.YM_pan$y
mcar.YM_pan <- pan(y=YM_M1.mcar,subj,pred, xcol, zcol, prior_mcar,seed=i+989,iter=100,start=mcar.YM_pan$last)
mcar.YM9 <- mcar.YM_pan$y
mcar.YM_pan <- pan(y=YM_M1.mcar,subj,pred, xcol, zcol, prior_mcar,seed=i+979,iter=100,start=mcar.YM_pan$last)
mcar.YM10 <- mcar.YM_pan$y

d.mcar.Y1 <- data.frame(y=mcar.YM1[,1],subj,pred,m_m1=mcar.YM1[,2])
d.mcar.Y2 <- data.frame(y=mcar.YM2[,1],subj,pred,m_m1=mcar.YM2[,2])
d.mcar.Y3 <- data.frame(y=mcar.YM3[,1],subj,pred,m_m1=mcar.YM3[,2])
d.mcar.Y4 <- data.frame(y=mcar.YM4[,1],subj,pred,m_m1=mcar.YM4[,2])
d.mcar.Y5 <- data.frame(y=mcar.YM5[,1],subj,pred,m_m1=mcar.YM5[,2])
d.mcar.Y6 <- data.frame(y=mcar.YM6[,1],subj,pred,m_m1=mcar.YM6[,2])
d.mcar.Y7 <- data.frame(y=mcar.YM7[,1],subj,pred,m_m1=mcar.YM7[,2])
d.mcar.Y8 <- data.frame(y=mcar.YM8[,1],subj,pred,m_m1=mcar.YM8[,2])
d.mcar.Y9 <- data.frame(y=mcar.YM9[,1],subj,pred,m_m1=mcar.YM9[,2])
d.mcar.Y10 <- data.frame(y=mcar.YM10[,1],subj,pred,m_m1=mcar.YM10[,2])
...
rename_Y_dataset <- function(x) {
  names(x) <- c("Y", "id", "int","group","time.2","time.3","time.4","time.5","time.6","X1","X2",
                "grp_time.2","grp_time.3","grp_time.4","grp_time.5","grp_time.6","M_M1")
  return(x)
}
...
for(k in 1:10)
{
  ...
  data.mcar <- data.mcar[,c("Y", "id", "group","X1","X2","M_M1")]
  time <- data.complete$time
  data.mcar <- cbind(data.mcar, time)
  
  data.mcar_wide <- reshape(data.mcar, idvar = "id", timevar = "time", v.names=c("M_M1","Y"), 
                            direction = "wide")
  
  lm_mcar.M <- lm(M_M1.6 ~ group + X1 + X2, data = data.mcar_wide)
  lm_mcar.Y <- lm(Y.6 ~ group + M_M1.6 + X1 + X2, data = data.mcar_wide)
...
Y.mar <- as.vector(combinedData$Y.mar)
M_M1.mar <- as.vector(combinedData$M_M1.mar)
YM_M1.mar <- cbind(Y.mar, M_M1.mar)

# Using E-M algorithmn to get priors for error variance and RE variance

model.em_mar <- mlmmm.em(y=YM_M1.mar, subj, pred, xcol, zcol, maxits = 400, eps = 0.0001)
a = r = c = 2 
...
mar.YM_pan <- pan(y=YM_M1.mar,subj,pred, xcol, zcol, prior_mar,seed=i+387,iter=5000)
mar.YM1 <- mar.YM_pan$y
mar.YM_pan <- pan(y=YM_M1.mar,subj,pred, xcol, zcol, prior_mar,seed=i+579,iter=100,start=mar.YM_pan$last)
mar.YM2 <- mar.YM_pan$y
mar.YM_pan <- pan(y=YM_M1.mar,subj,pred, xcol, zcol, prior_mar,seed=i+621,iter=100,start=mar.YM_pan$last)
mar.YM3 <- mar.YM_pan$y
mar.YM_pan <- pan(y=YM_M1.mar,subj,pred, xcol, zcol, prior_mar,seed=i+467,iter=100,start=mar.YM_pan$last)
mar.YM4 <- mar.YM_pan$y
mar.YM_pan <- pan(y=YM_M1.mar,subj,pred, xcol, zcol, prior_mar,seed=i+612,iter=100,start=mar.YM_pan$last)
mar.YM5 <- mar.YM_pan$y
mar.YM_pan <- pan(y=YM_M1.mar,subj,pred, xcol, zcol, prior_mar,seed=i+787,iter=100,start=mar.YM_pan$last)
mar.YM6 <- mar.YM_pan$y
mar.YM_pan <- pan(y=YM_M1.mar,subj,pred, xcol, zcol, prior_mar,seed=i+489,iter=100,start=mar.YM_pan$last)
mar.YM7 <- mar.YM_pan$y
mar.YM_pan <- pan(y=YM_M1.mar,subj,pred, xcol, zcol, prior_mar,seed=i+825,iter=100,start=mar.YM_pan$last)
mar.YM8 <- mar.YM_pan$y
mar.YM_pan <- pan(y=YM_M1.mar,subj,pred, xcol, zcol, prior_mar,seed=i+751,iter=100,start=mar.YM_pan$last)
mar.YM9 <- mar.YM_pan$y
mar.YM_pan <- pan(y=YM_M1.mar,subj,pred, xcol, zcol, prior_mar,seed=i+822,iter=100,start=mar.YM_pan$last)
mar.YM10 <- mar.YM_pan$y

d.mar.Y1 <- data.frame(y=mar.YM1[,1],subj,pred,m_m1=mar.YM1[,2])
d.mar.Y2 <- data.frame(y=mar.YM2[,1],subj,pred,m_m1=mar.YM2[,2])
d.mar.Y3 <- data.frame(y=mar.YM3[,1],subj,pred,m_m1=mar.YM3[,2])
d.mar.Y4 <- data.frame(y=mar.YM4[,1],subj,pred,m_m1=mar.YM4[,2])
d.mar.Y5 <- data.frame(y=mar.YM5[,1],subj,pred,m_m1=mar.YM5[,2])
d.mar.Y6 <- data.frame(y=mar.YM6[,1],subj,pred,m_m1=mar.YM6[,2])
d.mar.Y7 <- data.frame(y=mar.YM7[,1],subj,pred,m_m1=mar.YM7[,2])
d.mar.Y8 <- data.frame(y=mar.YM8[,1],subj,pred,m_m1=mar.YM8[,2])
d.mar.Y9 <- data.frame(y=mar.YM9[,1],subj,pred,m_m1=mar.YM9[,2])
d.mar.Y10 <- data.frame(y=mar.YM10[,1],subj,pred,m_m1=mar.YM10[,2])
...
for(m in 1:10)
{
  ...
  data.mar <- data.mar[,c("Y", "id", "group","X1","X2","M_M1")]
  time <- data.complete$time
  data.mar <- cbind(data.mar, time)
  
  data.mar_wide <- reshape(data.mar, idvar = "id", timevar = "time", v.names=c("M_M1","Y"), 
                            direction = "wide")
  
  lm_mar.M <- lm(M_M1.6 ~ group + X1 + X2, data = data.mar_wide)
  lm_mar.Y <- lm(Y.6 ~ group + M_M1.6 + X1 + X2, data = data.mar_wide)
...
mcar_wide_mice_lm2 <- mcar_wide_mice_lm[,c("group","X1","X2","mcar_M6_M1","Y.mcar.6")]
mar_wide_mice_lm2 <- mar_wide_mice_lm[,c("group","X1","X2","mar_M6_M1","Y.mar.6")]

mcar_mice_lm <- mice(mcar_wide_mice_lm2, m=numb_imput, maxit = 15,printFlag=FALSE, method = 'pmm', seed = 4+i)
mar_mice_lm <- mice(mar_wide_mice_lm2, m=numb_imput, maxit = 15,printFlag=FALSE, method = 'pmm', seed = 1400+i)

long.mcar_mice_lm_imp <- mice::complete(mcar_mice_lm, action='long', include=TRUE)
long.mar_mice_lm_imp <- mice::complete(mar_mice_lm, action='long', include=TRUE)
...
for (qq in 1:10)
{
  data.mcar <- long.mcar_mice_lm_imp[which(long.mcar_mice_lm_imp$.imp==qq), ]
  
  lm_mcar.M <- lm(mcar_M6_M1 ~ group + X1 + X2, data = data.mcar)
  lm_mcar.Y <- lm(Y.mcar.6 ~ group + mcar_M6_M1 + X1 + X2, data = data.mcar)

  ACME.mcar_mice_lm[qq] <- coefficients(lm_mcar.M)[2] * coefficients(lm_mcar.Y)[3]
  ADE.mcar_mice_lm[qq] <- coefficients(lm_mcar.Y)[2]  
  
  sobel_mcar_mice_lm[qq] <- sobel.test(a=coefficients(lm_mcar.M)[2], b=coefficients(lm_mcar.Y)[3], 
                                   var.a=diag(vcov(lm_mcar.M))[2], var.b=diag(vcov(lm_mcar.Y))[3])
  ADE.var_mcar_mice_lm[qq] <- diag(vcov(lm_mcar.Y))[2]
}
...
for (aa in 1:10)
{
  data.mar <- long.mar_mice_lm_imp[which(long.mar_mice_lm_imp$.imp==aa), ]
  ...
  ACME.mar_mice_lm[aa] <- coefficients(lm_mar.M)[2] * coefficients(lm_mar.Y)[3]
  ADE.mar_mice_lm[aa] <- coefficients(lm_mar.Y)[2]  
  
  sobel_mar_mice_lm[aa] <- sobel.test(a=coefficients(lm_mar.M)[2], b=coefficients(lm_mar.Y)[3], 
                                     var.a=diag(vcov(lm_mar.M))[2], var.b=diag(vcov(lm_mar.Y))[3])
  ADE.var_mar_mice_lm[aa] <- diag(vcov(lm_mar.Y))[2]  
}
...
mcar.data_mice0 <- combinedData[,c("group","time","X1","X2","M_M1.mcar","Y.mcar")]
mar.data_mice0 <- combinedData[,c("group","time","X1","X2","M_M1.mar","Y.mar")]
...
### Imputing missing for MCAR and MAR

mcar.mice_imputed <- mice(mcar.data_mice, m=numb_imput, maxit = 15,printFlag=FALSE, method = 'pmm', seed = 3390+i)
mar.mice_imputed <- mice(mar.data_mice, m=numb_imput, maxit = 15,printFlag=FALSE, method = 'pmm', seed = 7530+i)

## MCAR

long.mcar_mice <- mice::complete(mcar.mice_imputed, action='long', include=FALSE)
long.mcar_mice.single <- long.mcar_mice[ which(long.mcar_mice$time.6==1), ]
...
for (n in 1:10)
{
  data.mcar <- long.mcar_mice.single[which(long.mcar_mice.single$.imp==n), ]
  
  lm_mcar.M <- lm(M_M1.mcar ~ group + X1 + X2, data = data.mcar)
  lm_mcar.Y <- lm(Y.mcar ~ group + M_M1.mcar + X1 + X2, data = data.mcar)

  ACME.mcar_mice[n] <- coefficients(lm_mcar.M)[2] * coefficients(lm_mcar.Y)[3]
  ADE.mcar_mice[n] <- coefficients(lm_mcar.Y)[2]  
  
  sobel_mcar_mice[n] <- sobel.test(a=coefficients(lm_mcar.M)[2], b=coefficients(lm_mcar.Y)[3], 
                                  var.a=diag(vcov(lm_mcar.M))[2], var.b=diag(vcov(lm_mcar.Y))[3])
  ADE.var_mcar_mice[n] <- diag(vcov(lm_mcar.Y))[2]
}
...
long.mar_mice.single <- long.mar_mice[ which(long.mar_mice$time.6==1), ]
...
for (t in 1:10)
{
  data.mar <- long.mar_mice.single[which(long.mar_mice.single$.imp==t), ]
  
  lm_mar.M <- lm(M_M1.mar ~ group + X1 + X2, data = data.mar)
  lm_mar.Y <- lm(Y.mar ~ group + M_M1.mar + X1 + X2, data = data.mar)
  
  ACME.mar_mice[t] <- coefficients(lm_mar.M)[2] * coefficients(lm_mar.Y)[3]
  ADE.mar_mice[t] <- coefficients(lm_mar.Y)[2]  
  
  sobel_mar_mice[t] <- sobel.test(a=coefficients(lm_mar.M)[2], b=coefficients(lm_mar.Y)[3], 
                                   var.a=diag(vcov(lm_mar.M))[2], var.b=diag(vcov(lm_mar.Y))[3])
  ADE.var_mar_mice[t] <- diag(vcov(lm_mar.Y))[2]
}
...
save(est_miss_20180213, file = "C:/Users/name/R code/diffthensim_20180213.RData")
